...
static bool ui_mach_Interaction_flag = false;
/*
*此文件由GUI工具生成
*文件功能：用于处理用户的逻辑相应代码
*功能说明：
...
static void onUI_init(){
	...
	ui_mach_Interaction_flag = false;
}
...
static void onProtocolDataUpdate(SProtocolData &data) {
	...
	case CMDID_FRAMECHANGE:
		...
		case FRAME_MACHTESTING:
			...
			break;
		case FRAME_MACHREADY:
			EASYUICONTEXT->openActivity("FrameMachReadyActivity");
			break;
		...
		break;
	case CMDID_DISPLAYPOWERON:
		break;
	case CMDID_DISPLAYPOWERON_ACK:
		ui_mach_Interaction_flag = true;
		break;
	default:
		break;
	}
 ...
 *             停止运行当前定时器
 */
static const char *AnimationPrefix = "ui/animation/main_";
#define AnimationMaxNum		9
static bool onUI_Timer(int id){
	static int g_animationCnt = 0;
	char Animationpath[256] = {0};
	switch (id) {
	case 0:
		break;
	case 1:
		g_animationCnt = g_animationCnt%AnimationMaxNum;
		g_animationCnt +=1;
		sprintf(Animationpath,"%s%d.png",AnimationPrefix,g_animationCnt);
		mTextviewMainAnimationPtr->setBackgroundPic(Animationpath);
		if(g_animationCnt == AnimationMaxNum)
		{
			if(ui_mach_Interaction_flag == false)
				sendProtocol(CMDID_DISPLAYPOWERON, NULL, 0);
		}
		break;
//		if(g_animationCnt > 3)
//		{

 ...
 *             停止运行当前定时器
 */
static const char *AnimationPrefix = "ui/animation/loading_";
#define AnimationMaxNum  15
static bool onUI_Timer(int id){
	...
	case 0:
		break;
	case 1:
		sprintf(Animationpath,"%s%d.png",AnimationPrefix,g_animationCnt+1);
		g_animationCnt = (g_animationCnt + 1)%AnimationMaxNum;
		mTextviewMachCheckLoadingAnimalPtr->setBackgroundPic(Animationpath);
		break;
	default:
			break;
	}
 ...
 *            触摸事件将继续传递到控件上
 */
static bool onFrameMachCheckActivityTouchEvent(const MotionEvent &ev) {
